...
| `PAPERCLIP_LOCAL_AI_LOGIN_ENABLED` | `true` | `false` turns off the subscription sign-in that runs `claude auth login` (or `codex login`) in a terminal on the server. Use it on a hosted server with no terminal, such as Cloud Run. Trusted local MCP runtimes stay on. |
